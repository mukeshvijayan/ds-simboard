...
export function ESP32() {
  ...
  return (
    <svg viewBox="0 0 300 420" className="h-full w-full" role="img" aria-label="ESP32 development board">
      <rect x="20" y="10" width="260" height="400" rx="6" fill="#EDEBE3" stroke="#22314F" strokeWidth="1.5" />
      <text x="150" y="40" fontSize="14" fill="#22314F" fontFamily="var(--font-mono)" textAnchor="middle">
        ESP32 DEVKIT
      </text>
      ...
      {left.map((pin, i) => (
        <g key={`l-${pin}`}>
          <rect x="12" y={70 + i * 18} width="10" height="6" fill="#1C1B18" opacity="0.4" />
      ...
      {right.map((pin, i) => (
        <g key={`r-${pin}`}>
          <rect x="278" y={70 + i * 18} width="10" height="6" fill="#1C1B18" opacity="0.4" />

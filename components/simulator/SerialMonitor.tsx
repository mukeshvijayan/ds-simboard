...
export function SerialMonitor({ lines }: { lines: SerialLine[] }) {
  ...
  return (
    <div className="flex h-full flex-col border-t border-hairline bg-white">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-2 font-mono text-[12px] leading-relaxed text-charcoal">
        {lines.length === 0 && (
          <p className="text-charcoal-faint">Output from Serial.print() appears here once the sketch runs.</p>
        )}
        {lines.map((line) => (
